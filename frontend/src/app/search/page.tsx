  ...
  return (
    <div className="relative mb-8 overflow-hidden rounded-2xl border border-brand-red/30 bg-[#0f0f0f]">
      ...
      <div className="relative px-6 py-10 md:px-10 md:py-12">
        ...
        <h1 className="font-heading mb-3 text-4xl font-extrabold leading-tight tracking-tight text-white md:text-5xl lg:text-6xl">
          ...
          <span className="relative">
            ...
            <span className="absolute -bottom-1 left-0 h-0.5 w-full bg-brand-red/50 rounded" />
          </span>
        </h1>

        <p className="mb-8 max-w-md text-base text-neutral-400">
          Fresh styles added every week. Be the first to wear what&apos;s new — from
          statement dresses to everyday essentials.
        </p>
      ...
      <div className="absolute right-0 top-0 h-14 w-14 overflow-hidden">
        <div className="absolute right-[-14px] top-[-14px] h-14 w-14 rotate-45 bg-brand-red" />
        <span className="absolute right-2 top-2 text-[10px] font-bold text-white uppercase rotate-0 select-none">
          NEW
        </span>
      </div>
    </div>
        ...
        <div className="flex flex-wrap gap-2">
          {brandLinks.map((brand) => (
            <Link
              ...
              className="group flex items-center gap-1.5 rounded-full border border-neutral-700 bg-neutral-900 px-4 py-2 text-xs font-bold uppercase tracking-[0.15em] text-neutral-300 transition-all duration-200 hover:border-brand-red hover:bg-brand-red/10 hover:text-white"
            >
              {brand.name}
      ...
      <div className="overflow-hidden border-t border-neutral-800 py-3">
        <div className="flex min-w-max animate-marquee gap-8 px-6 opacity-30">
          {[...brandLinks, ...brandLinks].map((b, i) => (
            <span
              key={i}
              className="text-[10px] font-black uppercase tracking-[0.3em] text-white whitespace-nowrap"
            >
              {b.name}
            </span>
          ))}
...
export default async function SearchPage({
  ...
  return (
    <div>
      ...
      {/* Results count (only for user search queries) */}
      {searchValue && !isBrands ? (
        <p className="mb-6 text-sm text-neutral-400">
          {products.length === 0 ? (
            ...
            <>
              Showing{" "}
              <span className="text-white">{products.length} {resultsText}</span> for{" "}
